...
# Строка 1: дата, время, категория, СУММА В ВАЛЮТЕ ОПЕРАЦИИ, [остаток средств].
# В выписке ДВЕ колонки чисел: сумма операции и остаток на счёте. Берём ПЕРВУЮ
# (сумму операции), остаток — опциональный хвост, который игнорируем. Иначе в БД
# попадал running balance вместо суммы (599 ₽ превращались в остаток 14 357 ₽).
_TX_LINE1 = re.compile(
    ...
    r"\s{2,}"  # отступ перед суммой операции
    r"(\+?\d[\d ]*,\d{2})"  # СУММА ОПЕРАЦИИ: необязательный +, цифры, запятая, 2 знака
    r"(?:\s{2,}\d[\d ]*,\d{2})?"  # остаток средств (опционально) — игнорируем
    r"\s*$"
)
